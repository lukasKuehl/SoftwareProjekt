...
class TauschanfrageStrg {
	...
	protected boolean erstelleTauschanfrage(String senderName, int senderSchichtNr, String empfaengerName,
		...
		// Überprüfen ob die Übergebenen Variablen leer sind, wenn nicht
		// weiterleiten an den nächsten Test und valid auf true setzen
		if (senderSchicht != null && empfaengerSchicht != null && sender != null && empfaenger != null) {
			valid = true;
			// Überprüfen ob es zu den Übergebenen Werten bereits eine
			// Tauschanfrage gibt, wenn ja setze valid auf false
			for (Tauschanfrage ta : alleTauschanfragen) {
				if (ta.getEmpfänger().equals(empfaengerName) && ta.getSender().equals(senderName)
						&& ta.getSchichtnrempfänger() == empfaengerSchichtNr
		...
		// Tauschanfrage hinzugefügt werden
	...
	/**
	 * @Anes Preljevic
	 * @info Der Empfänger einer Tauschanfrage möchte diese annehmen, um
	...
	protected boolean akzeptiereTauschanfrage(String empfaengerName, int tauschanfrageNr) {
		...
		// Falls die Tauschanfrage nicht vorhanden ist wird die Bestätigung über
		// "valid" nicht gegeben, da es false bleibt
		for (Tauschanfrage ta : alleTauschanfragen) {
			if ((ta.getTauschnr() == tauschanfrageNr) && (ta.getEmpfänger().equals(empfaengerName))) {
				System.out.println(ta.getTauschnr()+""+ta.getEmpfänger());
				// Überprüfen ob die Mitarbeiter tatsächlich in den Angegebenen
				// Schichten sind
				for (Ma_Schicht masch : maschichtList) {
					
					// Gibt es eine Ma_Schicht-Relation in welcher, der
					// Empfänger in der übergebenen Schicht zu finden ist?
					if (masch.getBenutzername().equals(ta.getEmpfänger())
							&& masch.getSchichtnr() == ta.getSchichtnrempfänger()){
						System.out.println(masch.getBenutzername()+";"+ta.getEmpfänger()+";"+masch.getSchichtnr()+""+ ta.getSchichtnrempfänger());
						valid = true;
					}
					// Gibt es eine Ma_Schicht-Relation in welcher, der Sender
					// in der übergebenen Schicht zu finden ist?
		...
		// die Variablen übergeben und die Tauschanfrage bestätigt
		if (valid && valid2) {
			try {
		...
		// Fehlermeldung, dass es zu dem übergebenen Empfänger und der Tauschnr
		// keine gültige Tauschanfrage gibt
		else {
	 ...
	 *       gerichtet sind (Mitarbeiter ist somit als Empfänger eingetragen)
	 *       und Aufbereitung für Anzeige in der View
	 */
	protected ArrayList<String> getTauschanfragen(String username) {
		...
		// Suche alle Tauschanfragen, die für den übergebenen Mitarbeiternamen
		// bestimmt sind, für die die Person als Empfänger eingetragen ist.
		for (Tauschanfrage ta : alleTauschanfragen) {
		...
		// Generiere für jede Tauschanfrage eine Zeile nach folgendem Muster
		// Sendername - SenderSchichtNr - KW - TagSender - ZeitraumSender ||
		// Empfängername - EmpfängerSchichtNr - KW - TagEmpfänger -
		// ZeitraumEmpfänger
